...
### Если «ползунок громкости не влияет на игру»

Игра в Wine идёт через `winepulse`, то есть системная громкость ей подчиняется. Обычная
причина обратного — регулируется не тот выход: `pactl get-default-sink` показывает текущий,
и ползунок в панели управляет именно им. Готовый скрипт: `scripts/10-audio.sh`.

У Bluetooth-колонок аппаратной громкости обычно нет, работает программная; никакие
`module-combine-sink` для этого не нужны — PipeWire отдаёт громкость Bluetooth сам.
...
Встроенная панель 800x480 меньше минимума игры (800x600). Лаунчер игры решает это так:
на время игры поднимает масштаб до **0.8** (логически 1003x602) и уменьшает окно игры до
1000x600, а при выходе возвращает нормальные 800x480. Так и система остаётся удобной,
и игре хватает места.

## Шаг 8. Скорость боя и карты (по вкусу)

Мод добавляет настройки скорости, но **меню игры их не применяет**: меню пишет в секцию
`[Settings]` конфига, а мод читает свою `[Wrapper]`. Менять надо скриптом:

```bash
./09-game-speed.sh show            # что сейчас
./09-game-speed.sh battle 2        # скорость боя
./09-game-speed.sh map 5           # скорость передвижения по карте
./09-game-speed.sh both 2 5        # обе сразу
```

Шкала 1–20, но рабочая зона в начале: 1 — медленно, а 10 и 20 — уже быстро.
Разумная отправная точка — бой 2, карта 5. Применяется после **перезапуска игры**.
В бою скорость можно переключать клавишей (по умолчанию — 7).
...
PANEL_SCALE="${PANEL_SCALE:-1}"     # система всегда в нормальном масштабе;
                                     # нужный игре 0.75 поднимает лаунчер disciples2.sh
...
### 9. Скорость боя и карты

Файл: `~/d2-scripts/09-game-speed.sh`

```bash
mkdir -p $(dirname $HOME/d2-scripts/09-game-speed.sh)
cat > $HOME/d2-scripts/09-game-speed.sh << 'SCRIPT_EOF'
#!/bin/bash
...
SCRIPT_EOF
chmod +x $HOME/d2-scripts/09-game-speed.sh 2>/dev/null || true
```

### 10. Звук: громкость и выбор выхода

Файл: `~/d2-scripts/10-audio.sh`

```bash
mkdir -p $(dirname $HOME/d2-scripts/10-audio.sh)
cat > $HOME/d2-scripts/10-audio.sh << 'SCRIPT_EOF'
#!/bin/bash
...
SCRIPT_EOF
chmod +x $HOME/d2-scripts/10-audio.sh 2>/dev/null || true
```

### 11. Запуск игры (ярлык)
